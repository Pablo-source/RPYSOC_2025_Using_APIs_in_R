...
## Obtain the API response as a list
...
- For that we will use the glimpse() function
...
National_weather %>% resp_body_json() # Display JSON file contents
```
...
National_weather %>% resp_body_json() %>% # Display JSON file contents using glipmse()
  glimpse() 
```
