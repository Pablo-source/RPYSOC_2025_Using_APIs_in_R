...
## How to make a request through an API

- Obtain weather data using URL. From the US National Weatehr service
- We pass the request to the req_* function
...
- Following example from the above website: For example: https://api.weather.gov/points/39.7456,-97.0892

```{r initial weather data API}
library(httr2)

base_url <- 'https://api.weather.gov' # Base URL 

request(base_url) %>%  # Append now further parts of the URL. Makes a GET() request to the API
  req_url_path_append(
    'points',
    '38.8894,-77.0352'
...
```{r API request example using httr2, echo = TRUE}
